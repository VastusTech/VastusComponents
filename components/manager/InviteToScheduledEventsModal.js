...
class InviteToScheduledEventsModalProp extends Component {
  ...
  render() {
            ...
            <Fragment>
              <Card fluid raised>
                <Card.Content>
                  ...
                  <Button loading={isInviteLoading} primary fluid onClick={() => {
                    eventInviteHandler(events[i])
    ...
    if (this.props.user.scheduledEvents && this.props.user.scheduledEvents.length && this.props.user.scheduledEvents.length > 0) {
      return (
        <Modal centered dimmer='blurring' size='large' open={this.props.open} onClose={this.props.onClose.bind(this)}
               closeIcon>
