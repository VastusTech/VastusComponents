...
const queryPosts = (filter, nextToken, isFinished, friends, fetchPostQuery, fetchClient, fetchTrainer, fetchEvent,
                    fetchChallenge, fetchPost, setIsLoading, setIsFinished, setNextToken, setPosts, postQueries,
                    putPostQuery) => {
    if (!isFinished) {
        setIsLoading(true);
        debugAlert("Fetching Post Feed Query");
        fetchPostQuery(PostCard.fetchVariableList, filter, postFeedLength, nextToken, (data) => {
            if (!data.nextToken) {
                setIsFinished(true);
            }
            if (data.items) {
                for (let i = 0; i < data.items.length; i++) {
                    ...
                    if (post.access === "public" || friends.includes(post.by)) {
                        ...
                        if (aboutItemType === "Client") {
                            ...
                            fetchChallenge(about, ChallengeCard.fetchVariableList);
    ...
    useEffect(() => {
        if (props.user.id) {
            ...
            queryPosts(props.filter, nextToken, isFinished, props.user.friends, props.fetchPostQuery, props.fetchClient, props.fetchTrainer, props.fetchEvent, props.fetchChallenge,
                props.fetchPost, setIsLoading, setIsFinished, setNextToken, setPosts, props.cache.postQueries,
                props.putPostQuery);
        }
    }, [props.user.id]);
