...
import {setBoardRead, peekAtFirstMessageFromBoard, queryNextMessagesFromBoard, discardBoard, addMessageFromNotification,
    addMessageToBoard
} from "../messageActions";
    ...
    describe("Add Message from Notification", () => {
        it("Should add message from notification", (done) => {
            reduxStore.dispatch(addMessageFromNotification("BOARD", {message: "MESSAGE"})).then(() => {
                expect(reduxStore.getActions()).excluding('asyncDispatch').to.eql([
                    { type: 'SET_IS_LOADING' },
                    { payload: { message: { message:"MESSAGE", messageURL:"not_found.png",
                                profilePicture:"roundProfile.png" }, board: 'BOARD' }, type: 'ADD_MESSAGE' },
                    { type: 'SET_IS_NOT_LOADING' }
                ]);
                done();
            });
